...
public final class BlockboxNet {
  ...
  public static final class Packet {
    ...
    public String[] parts() { return parts; }
  ...
  public static String playerToken(String name, int color, int colorCount) {
    return safeName(name) + ":" + Math.floorMod(color, colorCount);
  }

  public static String pos(String name, float x, float y, float z, float yaw, float pitch, int color, int colorCount) {
    return "POS|" + safeName(name) + "|" + floatText(x) + "|" + floatText(y) + "|" + floatText(z) + "|" + floatText(yaw) + "|" + floatText(pitch) + "|" + Math.floorMod(color, colorCount);
  }

  public static String block(int x, int y, int z, int blockId) {
    return "BLOC|" + x + "|" + y + "|" + z + "|" + blockId;
  }

  public static String block(int x, int y, int z, int blockId, int waterLevel) {
    return waterLevel > 0 ? BLOC + "|" + x + "|" + y + "|" + z + "|" + blockId + "|" + clamp(waterLevel, 1, 8) : block(x, y, z, blockId);
  }
  ...
  public static String world(long seed, float x, float y, float z, int color, String hostName, int hostColor, String sessionId) {
    return WORLD + "|" + seed + "|" + floatText(x) + "|" + floatText(y) + "|" + floatText(z) + "|" + color + "|" + safeName(hostName) + "|" + hostColor + "|" + escape(sessionId) + "|" + PROTOCOL_VERSION;
  }
  ...
  public static String helo(String playerName, String modHash) {
    ...
    return HELO + "|" + PROTOCOL_VERSION + "|" + safeName(playerName) + "|" + escape(hash);
  }

  public static String error(String code, String message) {
    return ERR + "|" + escape(code == null ? "ERROR" : code) + "|" + escape(message == null ? "" : message);
  }
